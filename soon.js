...
const soonColumn = document.getElementById('soon-column');
...
// Afficher uniquement les tâches à venir (ni aujourd'hui, ni demain, ni en retard)
function renderSoonTasks() {
  // Vider la colonne
  soonColumn.innerHTML = "";
  ...
  // Filtrer pour n'obtenir que les tâches à venir qui ne sont pas terminées
  const soonTasks = [];
  ...
  tasks.forEach((task, index) => {
    ...
    // Inclure seulement les tâches qui sont dans le futur (pas aujourd'hui, pas demain, pas en retard)
    if (taskDate > dayAfterTomorrow && !isDatePassed(task.deadline)) {
      soonTasks.push({task, index});
    }
  });
  
  // Vérifier s'il y a des tâches à venir
  if (soonTasks.length === 0) {
    // Aucune tâche à venir, afficher le message
    ...
    soonColumn.appendChild(emptyMessage);
    return;
  }
  
  // Trier les tâches par date (les plus proches d'abord)
  soonTasks.sort((a, b) => {
  ...
  soonTasks.forEach(({task, index}) => {
    renderTaskCard(task, index, soonColumn);
  });
